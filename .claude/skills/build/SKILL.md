...
`kit/skills/` contains skill folders and nothing else. Every one carries this library's `hof-` prefix, and what the library holds is the conventions and procedures for Furo/Nuxt apps.

The sibling libraries — `hora-skills-ort-core` (`hoc-`) and `hora-skills-ort-renchan` (`hor-`) — lay their own `kit/skills/` out the same way, so a skill moves between them by moving its folder, and the build and the audit read the same tree in every one of them.
...
A skill folder may hold its own subdirectories (`references/`, `scripts/`), but no `SKILL.md` below its top level — those subdirectories are the skill's own files, never more skills. There are no intermediate grouping directories: no `nuxt-vue/components/`, no `css/props/`.
...
```
kit/skills/hof-cp-table/   name: hof-cp-table   →   dist/skills/hof-cp-table/
```
...
- No source note is appended. The source path is `kit/skills/<name>/`, and the prefix names the library, so an installed `hof-cp-table/` already says where it came from. A footer repeating it would be text to maintain that carries nothing.
