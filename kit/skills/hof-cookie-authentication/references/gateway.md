...
A public route is the sign-in path or one flagged `skipFilter` (furo `FuroMeta`). The sign-in path is a constant (`ROUTE_PATH.SIGN_IN`), not a literal, so it lives in one place ([[hof-nuxt]] for app constants and middleware).
