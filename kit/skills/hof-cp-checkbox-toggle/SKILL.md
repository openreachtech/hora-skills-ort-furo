...
- Picking a value from a list rather than a plain boolean → use `hof-cp-select`.
- A momentary action trigger (submit, delete, open dialog) rather than a persistent on/off state → use `hof-cp-button`.
