...
At the bottom of the file, define `<Name>ContextParams` (extends `BaseFuroContextParams`) and `<Name>ContextFactoryParams` (alias of Params, or `RequiredExcept<Params, 'optionalField'>`). `@import` blocks pull types from `#app`, `vue`, `vue-router`, sibling `./index.vue`, fetchers, and GraphQL payload/capsule modules. See [[hoc-jsdoc]].
