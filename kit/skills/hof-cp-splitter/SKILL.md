...
- A show/hide region that collapses to nothing (not a fixed-size resizable pane) → use `hof-cp-collapsible` instead.
- Tabular data with its own scroll and pagination behavior → use `hof-cp-table` instead of wrapping it manually in `FuroScrollArea`.
- Swapping which content is visible via a tab strip, rather than showing both regions side by side → use `hof-cp-tabs` instead of `FuroSplitter`.
- A floating panel anchored to a trigger element → use `hof-cp-popover` instead; `FuroSplitter`/`FuroScrollArea` are for fixed in-flow layout, not overlays.
