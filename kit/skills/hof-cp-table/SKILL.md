...
- A single value that toggles between display and edit inline (not a whole table row) → use `hof-cp-editable-field`.
- Multi-step progress display, not row data → use `hof-cp-stepper`.
- Need a floating panel or hover hint inside a cell → use `hof-cp-popover`, and put it inside the `cell` slot rather than rebuilding table markup.
- Need a modal to confirm a row action (e.g. delete) → use `hof-cp-dialog` (`FuroAlertDialog`), triggered from the `cell` slot's action button.
