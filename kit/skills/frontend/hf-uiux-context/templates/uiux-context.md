...
> `hf-uiux-forge` (generator) and `hf-uiux-audit` (reviewer) skills read this file to understand
> the *whole* application, so when you later hand `hf-uiux-forge` a small raw requirement
...
> `hf-uiux-audit` gains its scope-compliance, audience-fit, and expectation-fit sections
> instead of marking them "not assessable". The `hf-uiux-context` skill can create and fill
> this file for you.
>
