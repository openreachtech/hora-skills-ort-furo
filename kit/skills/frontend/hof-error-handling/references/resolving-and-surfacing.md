...
Resolution happens in one place on the capsule. `isUnauthenticated()` compares the code directly against the `Unauthenticated` entry. Always resolve via the capsule method — never hand-map codes in a context. See [[hof-graphql]].
