...
Animate only `transform` and `opacity` here — both run on the compositor and skip layout/paint. If the `transform` scale causes a 1px sub-pixel jump, promote the element with `will-change: transform` (see [[hf-css]] transitions).
