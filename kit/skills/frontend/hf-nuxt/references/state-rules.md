...
- **Typedefs at the bottom**: `<Domain>Store` (return shape), `<Domain>State` (state shape), plus one typedef per params object. GraphQL-derived types come from the global `schema.graphql.*` namespace (see [[hf-graphql]]).
