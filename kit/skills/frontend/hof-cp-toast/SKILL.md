...
- A persistent inline validation error under a field → use
  `hof-cp-control-block` instead, not a toast.
- A modal that blocks interaction until the user responds → use
  `hof-cp-dialog` instead.
- A floating panel anchored to a specific trigger element (not a
  viewport-corner stack) → use `hof-cp-popover` instead.
- A progress indicator for a multi-step flow → use `hof-cp-stepper`
  instead.
