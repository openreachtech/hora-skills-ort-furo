...
The leading `export {}` makes the file a module so `declare global` applies. This is why every `.js` file can reference these as `schema.graphql.<TypeName>` **without any import** — they're global. See [[hf-nuxt]] for the other ambient `.d.ts` files.
