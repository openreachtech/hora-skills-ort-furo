...
- Needs a menu of discrete actions (not a click-anchored surface with slot content) → use `hf-cp-dropdown-menu`.
- Content must block the whole page or demand an explicit decision → use `hf-cp-dialog` (`FuroDialog` / `FuroAlertDialog`), not a modal `FuroPopover`.
- Needs a persistent side panel → use `hf-cp-dialog` (`FuroDrawer`).
- Plain collapsible section inline in the page flow (no floating/anchored positioning) → use `hf-cp-collapsible`.
- Just want a static text hint with no hover behavior (e.g. always-visible helper text next to a field) → use `hf-cp-control-block`'s hint area, not `FuroTooltip`.
