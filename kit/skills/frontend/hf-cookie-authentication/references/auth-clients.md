...
Three operations, each a Launcher / Payload / Capsule trio on the app base classes — the same pattern as any GraphQL client ([[hf-graphql]]). Only the auth-specific bits are here.
...
The endpoint is the app's audience endpoint (for example `/graphql-customer`); the backend side of these three operations is [[hb-cookie-authentication]].
