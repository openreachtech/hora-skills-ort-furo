...
- Plain multi-line text with no formatting (comment box, description field)
  → use `hf-cp-textarea` instead — it's lighter and string-only.
- A single-line input (name, email, search) → use `hf-cp-text-field`
  instead.
- Needs a label / hint / error message wrapper around it → wrap in
  `FuroControlBlock` (see `hf-cp-control-block`), don't hand-roll a
  `<label>`.
- In-place editing of a single already-displayed value (click to edit, no
  rich formatting) → use `hf-cp-editable-field` instead.
