...
- A menu of actions to trigger (not a value to store) → use `hof-cp-dropdown-menu`.
- A simple boolean choice → use `hof-cp-checkbox-toggle`.
- Needs a label / hint / error message around it → wrap in `FuroControlBlock` (see `hof-cp-control-block`), don't hand-roll a `<label>`.
- Grouped toolbar-style toggle buttons rather than a value-bearing field → use `hof-cp-toggle-group`.
