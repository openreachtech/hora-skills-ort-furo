...
- Needs a label / hint / error message wrapper around it → wrap in `FuroControlBlock` (see `hof-cp-control-block`), don't hand-roll a `<label>`.
- You only need an inline click-to-edit display/edit toggle for an arbitrary value (not specifically a date/time widget) → see `hof-cp-editable-field`.
- The trigger for opening a floating panel is a plain button/icon with arbitrary content (not a date field) → see `hof-cp-popover` or `hof-cp-dropdown-menu`.
