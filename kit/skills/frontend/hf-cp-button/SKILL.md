...
- You need a menu of multiple actions behind a single trigger → use `hf-cp-dropdown-menu` (its default trigger slot is already `FuroButton`-styled; don't nest a separate `FuroButton` inside it unless overriding the trigger content).
- You need a floating tooltip/panel anchored to a trigger → use `hf-cp-popover`.
- You need a modal/drawer opened from a click → use `hf-cp-dialog` for the panel itself; `FuroButton` is still the right choice for the element that opens it.
- You need a toggle that holds boolean state (pressed/unpressed, checked/unchecked) rather than firing a one-shot action → use `hf-cp-checkbox-toggle` or `hf-cp-toggle-group`.
- You need an inline value editor, not an action → use `hf-cp-editable-field`.
