...
`UserInterfaceState` and `ErrorMessageHash` typedefs are **exported from `index.vue`** and imported by the sibling `.js` files via JSDoc `@import` — see [[hof-error-handling]] and [[hoc-jsdoc]].
