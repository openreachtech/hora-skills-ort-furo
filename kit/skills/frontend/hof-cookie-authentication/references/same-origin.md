...
furo's `BaseGraphqlLauncher.endpointUrl` returns `config.ENDPOINT_URL` verbatim and hands it to `fetch`, so a relative value just works — only the config value changes ([[hof-furo-env]]).
...
TLS is required whenever the cookie is `Secure` — over plain HTTP the browser drops it. The backend's cookie flags (`SameSite=Lax`, `Secure`) are set in [[hor-cookie-authentication]].
