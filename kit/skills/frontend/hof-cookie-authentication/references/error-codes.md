...
These two checks drive the request runner ([request-healing](./request-healing.md)): `isUnauthenticated()` → renew and retry, `isRefreshTokenReused()` → revoke. The codes must match the backend's ([[hor-cookie-authentication]]). For general error-to-message mapping in the UI, see [[hof-error-handling]].
