...
- Multi-line text (comments, descriptions) → use `hf-cp-textarea` (`FuroTextarea`) instead.
- Needs a label / hint / error message around it → wrap in `FuroControlBlock` (see `hf-cp-control-block`), don't hand-roll a `<label>`.
- Picking a value from a fixed or searchable list rather than typing free text → use `hf-cp-select` (`FuroSelect` / `FuroAutocompleteField`).
- An in-place edit of an already-displayed value → use `hf-cp-editable-field`.
- Date/time entry → use `hf-cp-date-time`, not `FuroTextField` with manual formatting.
