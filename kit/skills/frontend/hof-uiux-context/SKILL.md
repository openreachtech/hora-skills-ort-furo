...
`hof-uiux-forge` generator reads it so a small raw request ("add a filters row") is built against
the real app type, scope, stack, and client rules; the `hof-uiux-audit` reviewer reads it so its
...
far as the project allows** — it never builds or audits UI (hand off to `hof-uiux-forge` /
`hof-uiux-audit` for that).
...
- `hof-uiux-forge` or `hof-uiux-audit` looked for a context file (`uiux-context.md`, or any
...
- If the user came here to build UI, hand off to **`hof-uiux-forge`** (now that the context
  exists, it will read it automatically).
- If the user came here to review UI, hand off to **`hof-uiux-audit`**.
- If `hof-uiux-forge` or `hof-uiux-audit` routed here, return to that skill and resume — it now has
