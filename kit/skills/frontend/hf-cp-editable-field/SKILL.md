...
- The value is always in edit mode (a normal form field, no preview state) → use `hf-cp-text-field` or `hf-cp-textarea` directly instead.
- You need a persistent labeled field wrapper (label/hint/error) around a normal input → use `hf-cp-control-block`.
- You need a floating panel/tooltip anchored to a trigger that isn't about editing a value → see `hf-cp-popover`.
- The interaction is picking a date/time rather than an arbitrary value → use `hf-cp-date-time`.
- You need a menu of discrete actions, not a value editor → use `hf-cp-dropdown-menu`.
