...
- A group of mutually-exclusive toggle buttons that don't each own a content panel (e.g. a view-mode toolbar) → use `hof-cp-toggle-group`.
- Multi-step linear flow with a progress indicator (not freely switchable tabs) → use `hof-cp-stepper`.
- A dropdown list of navigation actions rather than a persistent trigger row → use `hof-cp-dropdown-menu`.
