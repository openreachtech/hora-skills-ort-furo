...
- Simple hover/focus hint text → use `hof-cp-popover` (`FuroTooltip`).
- A menu of discrete actions anchored to a trigger → use `hof-cp-dropdown-menu`.
- Toast-style transient feedback (success/error message, not a decision) → use `hof-cp-toast`.
- Inline expand/collapse of content within the page flow (not an overlay) → use `hof-cp-collapsible`.
