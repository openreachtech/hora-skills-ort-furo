...
| `@openreachtech/hora-skills-ort-furo` (this one) | `hof-` | `frontend` | 46 |
| `@openreachtech/hora-skills-ort-support` | `hos-` | `support` | 3 |

[**Skill catalog**](https://github.com/openreachtech/hora-skills-ort-furo/blob/main/docs/skills.md) ([日本語](https://github.com/openreachtech/hora-skills-ort-furo/blob/main/docs/skills.ja.md)) — every skill in this package with a one- or two-line summary, listed by the command name it is invoked by.
...
{
  "scripts": {
    "postinstall": "hora-skills-ort-furo install"
  }
}
```
...
**`--no` stops npx before it downloads**: the name is still resolved against the registry, but nothing is fetched, so neither the install script nor the bin of a stranger's package ever runs. Without it, a bin that is not installed becomes a fetch of whatever has been published under `hora-skills-ort-furo`, an unscoped name this package does not hold.

Where the package is not a dependency at all — a one-off, or a repository that is not yours to add a hook to — name it in full instead: `npx --package=@openreachtech/hora-skills-ort-furo hora-skills-ort-furo install`. What is fetched is then a scoped name nobody else can publish under, which is the guarantee the unscoped bin name never carried.

## Usage

The skills land in your repository's `.claude/skills/`. Claude Code discovers them from there, and each becomes invocable by its own name — `/hof-cp-table`, `/hof-css-units`, `/hof-nuxt`. Installed skills sit side by side with your repository's own, in one flat list, which is what the `hof-` prefix is for.
...
{
  "scripts": {
    "postinstall": "hora-skills-ort-furo install && <another hora-skills package> install"
  }
}
```
...
`.claude/`, and the `skills/` directory inside it, have to be directories of your repository rather than symbolic links. An installation verifies every step it is reached through, and finding a link at any of them it writes nothing and removes nothing. `.hora/hora-skills-ort-furo.json`, the record of what was installed, is verified the same way: a link there would send the write to whatever it stands for and overwrite it.

An installation that carries nothing says why and ends with a failing exit code. Where the command runs as your project's own `postinstall`, that is what `npm install` reports; run on its own, `npx --no hora-skills-ort-furo install` tells you the same.
...
An `npm install` with no arguments re-runs your project's `postinstall`, so the skills follow along. Naming the package on the command line — `npm install @openreachtech/hora-skills-ort-furo@latest` — does not, and neither does a repository without a hook. Run the command again yourself:

```sh
npx --no hora-skills-ort-furo install
```

`install` is repeatable: it removes what the previous run installed — recorded in `.hora/hora-skills-ort-furo.json` — along with any folder named after a skill this package distributes, before copying the current output. A renamed or dropped skill therefore leaves nothing behind, and a repository that had copied `dist/skills/` by hand is tidied up on its first run.

A skill your own repository authored is left alone, as long as its name is not one this package distributes. Carrying the `hof-` prefix is not enough to put it at risk — `hof-own-skill` is untouched — but naming it exactly after a distributed skill hands that name over to this package.

### Commands

| Command | What it does |
| :-- | :-- |
| `hora-skills-ort-furo install` | Install every skill this package distributes, replacing the previously installed ones |
| `hora-skills-ort-furo list` | Print the skills this package distributes, installing nothing |
| `hora-skills-ort-furo uninstall` | Remove every skill this package installed, along with its record |
| `hora-skills-ort-furo help` | Print the usage text |
...
```sh
git clone https://github.com/openreachtech/hora-skills-ort-furo.git
cd hora-skills-ort-furo
npm install
npm run lint
npm test
```
