...
| `hof-lib-tokens` | 消費側アプリからコンポーネントライブラリ自身のセマンティックトークンを上書きする規約。公開されているプレフィックス、2 層のボーダー、intent のテキスト層、イージングトークン。 |
